...
class KeyRing:
    ...
    def token_digest(self, value: str) -> str:
        """Deterministic, one-way HMAC digest of a canonical value. Same value -> same digest
        within a session; different session key -> different digest (non-determinism across)."""
        self._check()
        return hmac.new(self._k_token, value.encode("utf-8"), hashlib.sha256).hexdigest()
