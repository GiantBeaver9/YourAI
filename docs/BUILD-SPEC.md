...
Presidio optional/pluggable. No secrets hardcoded — all via env/.env.
...
2. `Detector` (Protocol) — `RuleEngineDetector` (default) | `PresidioDetector` (optional).
...
2. **`vault/`** — DONE in foundation. `KeyRing` (HKDF→k_token/k_enc, AES-GCM, one-way digest),
   `SessionVault` (encrypted token→original), `Session`/`SessionManager` (lifecycle + leases).
...
4. **`detection/`** — `Detector` protocol + `RuleEngineDetector`:
   - Rules-as-data compiling to recognizers (Presidio `PatternRecognizer` when available, else native regex). Safe Harbor set: SSN/phone/email/MRN(label)/account/ZIP/etc.
   ...
   - `PresidioDetector` optional; guard the import so absence never breaks tests/demo.
...
8. **`store/`** — envelope encryption: master (env) → per-user KEK → per-doc DEK; AES-256-GCM with AAD=(user_id,doc_id); per-user-key isolation; file-based under `store_root`. Extracted plaintext never persisted unencrypted.
